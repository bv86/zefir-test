...
export const getRandomNumber = function getRandomNumber(
  min: number,
  max: number,
) {
...
export const fastFibonacci = async (num: number): Promise<number> => {
  const res = await new Array(num + 1).fill(1).reduce(async (prev, _, i) => {
...
export const writeAnagramsFile = function writeAnagramsFile(
  nbWords,
  filename,
  callback,
) {
